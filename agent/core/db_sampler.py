"""
V1.5 Agent - DBSmartSampler (Étape 5)

Zone-aware sampling pour databases
Copié depuis backend/src/profiling/db_sampler.py (V1.1)
...
GATE 5: 1000 tables → 400 sampled (-60%), 100% SENSITIVE conservées

Date: 2025-12-15
Version: 1.5.0 (Agent V1.5)
"""
...
# Utility function for quick zone check
def is_sensitive_table(table_name: str, schema: str = "") -> bool:
    """
    Quick check if table is sensitive (100% sampling required)
    ...
    Returns:
        True if table is sensitive, False otherwise
    """
    sampler = DBSmartSampler()
    return sampler.get_zone(table_name, schema) == "SENSITIVE"
